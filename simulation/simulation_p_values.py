import numpy as np
import matplotlib.pyplot as plt
from helpers import run_simulation_meng_our, run_simulation_our
import argparse
import os


# def compute_pvalue_data(n_values, T, discrete=True, alternative='two.sided'):
#     """
#     Compute p-values for different sample sizes.
    
#     Parameters:
#     -----------
#     n_values : list
#         List of sample sizes to test
#     T : int
#         Number of simulations to run
#     discrete : bool
#         Whether to use discrete simulation
#     alternative : str
#         Type of alternative hypothesis ('two.sided' or 'one.sided')
    
#     Returns:
#     --------
#     tuple
#         (list_of_pvals_meng, list_of_pvals_our) - arrays of p-values
#     """
#     num_n_values = len(n_values)
#     list_of_pvals_meng = np.zeros((num_n_values, T))
#     list_of_pvals_our = np.zeros((num_n_values, T))
    
#     # Run simulations for each sample size
#     for i, n in enumerate(n_values):
#         print(f"Processing n = {n}...")
        
#         if discrete:
#             # Use run_simulation_our for discrete case (only our approach)
#             pvals_our = run_simulation_our(
#                 n, T=T, discrete=discrete, alternative=alternative
#             )
#             list_of_pvals_our[i, :] = pvals_our
#         else:
#             # Use run_simulation_meng_our for continuous case (both approaches)
#             pvals_meng, pvals_our = run_simulation_meng_our(
#                 n, T=T, discrete=discrete, alternative=alternative
#             )
#             list_of_pvals_meng[i, :] = pvals_meng
#             list_of_pvals_our[i, :] = pvals_our
    
#     return list_of_pvals_meng, list_of_pvals_our

# def create_pvalue_histograms(n_values, list_of_pvals_meng, list_of_pvals_our, 
#                            discrete=True, alternative='two.sided', filename_suffix=None):
#     """
#     Create histograms from pre-computed p-values.
    
#     Parameters:
#     -----------
#     n_values : list
#         List of sample sizes
#     list_of_pvals_meng : numpy array
#         P-values from Meng method (shape: num_n_values x T)
#     list_of_pvals_our : numpy array
#         P-values from our method (shape: num_n_values x T)
#     discrete : bool
#         Whether data is from discrete simulation
#     alternative : str
#         Type of alternative hypothesis
#     filename_suffix : str, optional
#         Suffix to add to filename
#     """
#     num_n_values = len(n_values)
    
#     # Create figure with subplots
#     num_rows = 1 if discrete else 2
    
#     # Create figure with subplots
#     fig, axes = plt.subplots(num_rows, num_n_values, figsize=(20, 4 * num_rows))
    
#     # Ensure axes is 2D array even for single row or column
#     if num_rows == 1 and num_n_values == 1:
#         axes = np.array([[axes]])
#     elif num_rows == 1:
#         axes = axes.reshape(1, -1)
#     elif num_n_values == 1:
#         axes = axes.reshape(-1, 1)
    
#     # Create histograms for each sample size
#     for i, n in enumerate(n_values):
#         if not discrete:
#             # Create Meng histogram
#             axes[0, i].hist(
#                 list_of_pvals_meng[i, :], bins=20, density=True, alpha=1, color="darkgrey",
#                 edgecolor='black', linewidth=1.2, label='Meng Method'
#             )
#             axes[0, i].axhline(y=1, color='black', linestyle='--', linewidth=1)
#             axes[0, i].set_title(f'n = {n}')
#             axes[0, i].set_ylim(0, 1.2)
#             axes[0, i].set_xlabel('P-value')
#             axes[0, i].set_ylabel('Density')
#             axes[0, i].grid(True, alpha=0.3)
        
#         # Create Our method histogram
#         row_idx = 1 if not discrete else 0
#         axes[row_idx, i].hist(
#             list_of_pvals_our[i, :], bins=20, density=True, alpha=1, color="darkgrey",
#             edgecolor='black', linewidth=1.2, label='Our Method'
#         )
#         axes[row_idx, i].axhline(y=1, color='black', linestyle='--', linewidth=1)
#         axes[row_idx, i].set_title(f'n = {n}')
#         axes[row_idx, i].set_ylim(0, 1.2)
#         axes[row_idx, i].set_xlabel('P-value')
#         axes[row_idx, i].set_ylabel('Density')
#         axes[row_idx, i].grid(True, alpha=0.3)
    
#     # Add method labels
#     if not discrete:
#         axes[0, 0].text(-0.03, 1.05, 'Meng', transform=axes[0, 0].transAxes, 
#                        fontsize=14, verticalalignment='bottom', horizontalalignment='left')
#         axes[1, 0].text(-0.03, 1.05, 'Our', transform=axes[1, 0].transAxes, 
#                        fontsize=14, verticalalignment='bottom', horizontalalignment='left')
#     else:
#         axes[0, 0].text(-0.03, 1.05, 'Our', transform=axes[0, 0].transAxes, 
#                        fontsize=14, verticalalignment='bottom', horizontalalignment='left')
    
#     # Adjust layout
#     plt.tight_layout()
#     plt.subplots_adjust(top=0.93)
    
#     # Save as PDF
#     discrete_str = "discrete" if discrete else "real"
#     filename = f"pvalue_histograms_{discrete_str.lower()}_{alternative.replace('.', '_')}"
#     if filename_suffix:
#         filename += f"_{filename_suffix}"
#     filename += ".pdf"
#     plt.savefig('./' + filename, format='pdf', dpi=300, bbox_inches='tight')
    
#     return None


# def run_pvalue_analysis(n_values, T, discrete=True, alternative='two.sided', filename_suffix=None):

#     list_of_pvals_meng, list_of_pvals_our = compute_pvalue_data(
#         n_values, T, discrete, alternative
#     )

#     create_pvalue_histograms(
#         n_values, list_of_pvals_meng, list_of_pvals_our, 
#         discrete, alternative, filename_suffix
#     )
    
#     return list_of_pvals_meng, list_of_pvals_our


def run_single_job(n, T, discrete, alternative, output_dir):
    """
    Run simulation for a single n value and save results.
    
    Parameters:
    -----------
    n : int
        Sample size
    T : int
        Number of simulations
    discrete : bool
        Whether to use discrete simulation
    alternative : str
        Type of alternative hypothesis
    output_dir : str
        Directory to save results
    """
    print(f"Starting simulation: n={n}, T={T}, discrete={discrete}, alternative={alternative}")
    
    if discrete:
        # Discrete case - only our method
        pvals_our = run_simulation_our(
            n, T=T, discrete=discrete, alternative=alternative
        )
        pvals_meng = np.array([])  # Empty array for discrete
    else:
        # Continuous case - both methods
        pvals_meng, pvals_our = run_simulation_meng_our(
            n, T=T, discrete=discrete, alternative=alternative
        )
    
    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    
    # Save results
    discrete_str = "discrete" if discrete else "continuous"
    alt_str = alternative.replace('.', '_')
    
    filename_meng = os.path.join(output_dir, f"pvals_meng_{discrete_str}_{alt_str}_n{n}.npy")
    filename_our = os.path.join(output_dir, f"pvals_our_{discrete_str}_{alt_str}_n{n}.npy")
    
    if not discrete:
        np.save(filename_meng, pvals_meng)
    np.save(filename_our, pvals_our)


def main():
    parser = argparse.ArgumentParser(description='Run p-value simulation for single n')
    parser.add_argument('--n', type=int, required=True, help='Sample size')
    parser.add_argument('--T', type=int, default=100000, help='Number of simulations')
    parser.add_argument('--discrete', action='store_true', help='Use discrete simulation')
    parser.add_argument('--alternative', type=str, default='two.sided', 
                       choices=['two.sided', 'one.sided'],
                       help='Type of alternative hypothesis')
    parser.add_argument('--output_dir', type=str, default='./results',
                       help='Directory to save results')
    
    args = parser.parse_args()
    
    np.random.seed(42)
    run_single_job(args.n, args.T, args.discrete, args.alternative, args.output_dir)


if __name__ == "__main__":
    main()

    
# Configuration
# n_values = [50, 100, 500, 1000, 5000]
    
    
# Run discrete two-sided test
#    results_discrete_two = run_pvalue_analysis(
#        n_values=n_values,
#        T=T,
#        discrete=True,
#        alternative='two.sided'
#        )

    
# Run continuous one-sided test
#    results_continuous_one = run_pvalue_analysis(
#        n_values=n_values,
#        T=T,
#        discrete=False,
#        alternative='one.sided'
#    )

