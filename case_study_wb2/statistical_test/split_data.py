...
from config import FORECAST_NAMES, DATA_DIR, CHUNK_DIR, NUM_CHUNKS, OBS_PATH
...
def prepare_data_chunks(forecast_path, obs_path, forecast_names, n_chunks, chunk_dir='./chunks'):
    ...
    if var_obs.shape[1] != n_lat or var_obs.shape[2] != n_lon:
        print("Transposing data arrays from (time, lon, lat) to (time, lat, lon)")
        var_obs = np.transpose(var_obs, (0, 2, 1))  # (time, lon, lat) -> (time, lat, lon)
