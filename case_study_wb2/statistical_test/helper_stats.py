...
@njit()
def kernel_ties_optim1(x_rank_sort, y_rank_sort, rho, y_rank_unique, y_num, pos_y, ix_y, x_rank_unique):
    #y_rank_sort = y_rank[ix_y]
    #x_rank_sort = x_rank[ix_y]
...
def Sigma_fast_clean(y_rank, xarray_ranks):
    N = len(y_rank)
    k = xarray_ranks.shape[0]
...
def Sigma_fast_clean_time_series(y_rank, xarray_ranks):
    N = len(y_rank)
    k = xarray_ranks.shape[0]
    ...
    for j in prange(k):
        ...
        for h in range(1, N):
            if h <= b:
                ...
                c_k = 0
                for t in range(N-h):
                    a_k += K1_values[t] * K1_values[t + h]
                    b_k += K1_values[t]*K0_values[t+h] + K0_values[t]*K1_values[t + h]
                    c_k += K0_values[t]*K0_values[t+h]
                A_spear += omega * (2 / N)* a_k  
                B_both += omega * (1 / N)* b_k
                C_zeta +=  omega* (2 / N)* c_k

        var2 = factor * (
            A_spear  + (2 * rhos[j]*B_both ) / ( 1 - zeta_3Y) + 
            (rhos[j] ** 2 * C_zeta ) / ((1-zeta_3Y)**2)
        )


        S[j, j] = var / (4 * N) + var2 / (4 * N)
        phalf[j] = 1 - norm.cdf((cmas[j] - 0.5) / np.sqrt(var / (4 * N)))
        ...
        for i in prange(j + 1, k):
            ...
            for h in range(1, N):
                if h <= b:
                    ...
                    c_k = 0
                    for t in range(N-h):
                        a_k += k_p[t]*k_p2[t+h] + k_p[t+h]*k_p2[t]
                        b_k1 += k_p[t]*K0_values[t+h] + k_p[t+h]*K0_values[t]
                        b_k2 += k_p2[t]*K0_values[t+h] + k_p2[t+h]*K0_values[t]
                        c_k += K0_values[t]*K0_values[t+h]
                    A2_spear += omega * (1 / N)* a_k  
                    B_one += omega * (1 / N)* b_k1
                    B_two += omega * (1 / N)* b_k2
                    C_zeta +=  omega* (2 / N)* c_k

            var2 = factor * (
                A2_spear + (rhos[j] * B_one) / (1-zeta_3Y) + 
                (rhos[i] * B_two) / (1 - zeta_3Y) +
                (rhos[j] * rhos[i] * C_zeta) / ((1 - zeta_3Y) ** 2)
            )
            S[j, i] = S[i, j] = var / (4 * N) + var2 / (4 * N)
...
# def calc_pvalue_chi_our_R(aucs, S):
#     nauc = len(aucs)
...
def pairwise_testing_ci(nauc, S, aucdiff, conf_level):
...
def cma_test_new_fast2(y, x ,conf_level = 0.95):
    ##################
    # checking N > 3 #
    ##################
...
def cma_time_series(y, x ,conf_level = 0.95):
    ##################
    # checking N > 3 #
    ##################
    ...
    if x.ndim == 1:
        ...
        # compute score, variance estimation and p-value
        cmas, sd, p = one_dim_test_time_series(y_ranks, x_ranks)
    ...
    # multiple predictors for y:
        ...
        # single value testing and variance estimation: 
        cmas, S , phalf = Sigma_fast_clean_time_series(y_ranks, xarray_ranks)

        # global testing:
        z, p, cmadiff = calc_pvalue_chi_our(cmas, S)

        # pairwise testing:
        cis = pairwise_testing_ci(len(cmas), S, cmadiff, conf_level)
    
        #return test_multiple(cmas = cma_pd, differences = diff_pd, covariance = S, global_z = z, global_p = p)
        variance = S[0, 0]-S[0, 1]-S[1, 0] + S[1, 1]
        return cmas, phalf, cis.squeeze(), p, variance
